 ...
 * Time: 10:01
 */
class clevent extends clcommon
{
    ...
    { //console
        global $wpdb;
        $cl = $wpdb->prefix . 'contactlist';
        $instvalev = $this->instvalev();
        $rep = $instvalev['rep'];

        $set = $rwcu['settings'];
        $stdef = $this->inststat();
        $rwcu = $this->getcur();
        $rsct = $wpdb->get_results("select * from " . $cl . " where tp = 'ev' order by st, sttm", ARRAY_A);
        if ($rsct) {
            foreach ($rsct as $rwct) {
                ...
                switch ($rwct['st']) {
                    case 'E10'  : // announced
                    case 'E20'  : // confirmed
                        if ( $rwct['sttm'] < $rembef) { // herinneren
                            $hertel = 0;
                            $arr = json_decode($rwct['tx'], TRUE);
        ...
        $rsct = $wpdb->get_results("select * from " . $cl . " where tp = 'ct' order by st, sttm", ARRAY_A);
        foreach ($rsct as $rwct) {
            if ($rwct['st'] != $stold) {                // de level break
                $ct['des'] = $stdef[$rwct['st']]['des'];
                $stold = $rwct['st'];
            }
            $ct['nm'] = $rwct['nm'];
            $ct['em'] = $rwct['em'];
            $ct['btst'] = $stdef[$stold]['btst'];
            $ct['bttx'] = $stdef[$stold]['bttx'];
    ...
    public function vlog($id = "0")
    {
        ...
        foreach ($logs as $time => $status) {
            ...
            if (isset($stdef[$status]['des'])) {
                echo $stdef[$status]['des'] . '<br>';
            }
        }
    }
    ...
    public function admin($id = "0")
    {
        ...
        <form action=<?php echo $this->action; ?> method='post'>
            ...
            <p class="submit">
                <input type="submit" name="admin" id="admin" class="button button-primary" value="<?php echo __( 'save', 'edcl' ); ?>">
            </p>
        </form>
        <?php
    }
